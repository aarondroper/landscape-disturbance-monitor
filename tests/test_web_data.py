...
def test_source_context_reconciles_80_ids_and_80_by_10_records():
...
def test_five_decimal_coordinate_rounding_passes_without_simplification():
    context = web._source_context(web._source_paths(ROOT))
...
def test_coordinate_fallback_retries_once_at_six_decimals():
    context = web._source_context(web._source_paths(ROOT))
    calls = []
...
def test_both_coordinate_precisions_fail_loudly():
    context = web._source_context(web._source_paths(ROOT))
...
def test_build_outputs_are_compact_reconciled_and_deterministic(tmp_path: Path):
...
def test_build_does_not_change_source_hashes():
...
def test_source_copy_is_not_simplified_or_repaired():
    source = _load(SOURCE_GEOJSON)
